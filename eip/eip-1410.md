...
Aggregates a token holders balances across all tranches. Equivalent to `balanceOf` in the ERC-20/777 specification.
...
This function MUST revert if called by an address lacking the appropriate approval as defined by `isOperatorForTranche` or `isOperatorFor`.
...
Operators can be authorised at by individual token holders for either all tranches, or a specific tranche.
